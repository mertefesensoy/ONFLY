...
| G1 SoftFloat port (Spike S1) | Can GCCMVS build and run a SoftFloat release correctly? | The integer self-test **for the width the MVS engine uses** and TT-02 pass under GCCMVS — that is TT-01/32 and TT-02 while NR-03's fallback to Release 2c is in force, or TT-01 and TT-02 if a 64-bit-capable toolchain is ever found *(amended 2026-09-11 by D-118; originally "TT-01 and TT-02 pass under GCCMVS")* | Switch to SoftFloat 2c (D-06); record JCC scope (VL-03) |
