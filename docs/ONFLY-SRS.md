...
| **G — Transaction demonstrator (second MVP)** | ONFLY as a transaction under INTERCOMM on TK5; the flow shown on a real 3270; a live view of the network rendered on x86 from streamed engine output (D-126, D-127, D-128) | **E**; TBD-17 (3270 client), TBC-18 (licence), TBD-19 (streaming interface) |
...
| TBD-17 | A 3270 client for the development host | Phase G, D-127 | **Open.** Every MVS interaction so far has gone through the card reader, the printer and the Hercules console; this host has no 3270 client at all, and D-127's transaction flow cannot be shown or even driven without one |
| TBC-18 | Whether an ONFLY subsystem using INTERCOMM's macros and copybooks is a derivative work under its noncommercial-only clause | NFR-LIC-01, D-62, Phase G | **Open.** INTERCOMM is BSD-style from Tetragon Software LLC but restricted to noncommercial use, and the repository is MIT. To be settled **before** any transaction code is written, not after |
| TBD-19 | The streaming output interface: what the engine emits during a run, over what record, and how often | FR-SIM, IR-COM, D-128 | **Open.** The engine today returns spike counts and first-spike latencies at the end of a run only. D-128 requires intermediate state during a run, which is an output path that does not exist. Gate G3 must measure the real cost first: the Hercules estimate is two to three orders of magnitude slower than x86, which may force a much smaller network or a much shorter duration than the golden fixture uses |
