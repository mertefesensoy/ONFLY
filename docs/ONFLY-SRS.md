...
| P-07 | Source line length on MVS is an open question, to be settled by measuring what GCCMVS accepts once TK5 runs: 11 ONFLY sources exceed 80 columns and vendored SoftFloat cannot be reflowed without breaking the byte-identity of D-28 and D-35 | An ONFLY-only column rule would not cover the case that actually matters, and the answer is empirical |
