...
| `prep/acc4.py` | New. ACC-4 (with ACC-1 and ACC-2 previews) at the calibrated W_syn, 30 seeds per validation rate; `--variant right`, `phg9` or `tpgrn` re-emits the network with a D-176 stimulus subset for the low-rate investigation. |
