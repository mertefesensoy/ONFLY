...
def emit():
    ...
    for name, _mac, arr, cases, fn, _isshift in GROUPS:
        ...
        for a, b in cases:
            ...
            bh, bl = halves(b)
            rh, rl = halves(r)
            # Two lines, not one.  D-93: the TK5 card reader is defined
            # `ascii trunc`, and an 85-column card was measured arriving as
            # exactly 80 characters with no diagnostic at all.  One line per
            # vector is 85 columns, so every vector would lose its closing
            # brace on the way into MVS.
            w("    { 0x%08XU, 0x%08XU, 0x%08XU," % (ah, al, bh))
            w("      0x%08XU, 0x%08XU, 0x%08XU }," % (bl, rh, rl))
        w("};")
        w("")
